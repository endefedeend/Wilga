...
begin
  ...
  begin
    if idxOld >= 0 then Result := gSoundRegs[idxOld].curVol else Result := 0.0;
  end;
  ...
  begin
    if idxOld >= 0 then
    begin
      gSoundRegs[idxOld].curVol := Max(0.0, v);
      SetSoundVolume(oldH, gSoundRegs[idxOld].curVol);
    end;
  end;
  ...
  begin
    Result := gSoundRegs[idxNew].curVol;
  end;

  setterNew := procedure(v: Double)
  begin
    gSoundRegs[idxNew].curVol := Clamp(v, 0.0, 1.0);
    SetSoundVolume(newH, gSoundRegs[idxNew].curVol);
  end;
  ...
  TweenValue(getterOld, setterOld, 0.0, time, easeLinear,
    procedure
    begin
      if idxOld >= 0 then StopSoundEx(oldH);
    end);
